...
- The restored PROD entrypoint is a minimal real HTTP process at `drivers/os_driver.py`: it binds the resolved host/port, serves `/health`, `/capabilities`, and `/displays`, and returns explicit startup errors on bind/root failures instead of pretending green.
